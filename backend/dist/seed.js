...
const seed = async () => {
    ...
    try {
        // 1. Clean Database
        await db_1.default.newsletterBlast.deleteMany();
        await db_1.default.favorite.deleteMany();
        await db_1.default.offer.deleteMany();
        await db_1.default.booking.deleteMany();
        await db_1.default.savedSearch.deleteMany();
        await db_1.default.vehicle.deleteMany();
        await db_1.default.user.deleteMany();
        await db_1.default.globalSettings.deleteMany();
        console.log('Database cleaned. Setting up corporate parameters...');
        // 2. Global corporate settings
        const settings = await db_1.default.globalSettings.create({
            data: {
                id: 'global',
        ...
        const adminPass = await bcryptjs_1.default.hash('admin', salt); // Default admin password is 'admin'. Please change after first login
        const customerPass = await bcryptjs_1.default.hash('CustomerPass2026!', salt);
        ...
        const customer = await db_1.default.user.create({
            data: {
        ...
        for (const item of vehiclesData) {
            await db_1.default.vehicle.create({ data: item });
        }
