...
const drawVectorLogo = (doc, logoX, logoY) => {
    doc.save();
    // Outer shield shape
    doc.lineWidth(2);
...
const generatePDFReport = async (res, title, headers, rows, summaryText, logoUrl, colWidths) => {
    ...
    if (logoUrl) {
        if (logoUrl.startsWith('/uploads/')) {
            resolvedLogoPath = path_1.default.join(__dirname, '../../public', logoUrl);
        }
        else {
            resolvedLogoPath = path_1.default.join(__dirname, '../../public/uploads', logoUrl);
        }
        if (fs_1.default.existsSync(resolvedLogoPath)) {
            hasImageLogo = true;
        }
    }
    // Draw Logo (Left side)
    if (hasImageLogo) {
        try {
            const pngBuffer = await (0, sharp_1.default)(resolvedLogoPath).png().toBuffer();
    ...
    // Page Footer Text
    doc.fontSize(8).fillColor(darkGrey).text('CONFIDENTIAL REPORT - FOR J&L AUTOS INTERNAL ADMINISTRATIVE CRM ROLES ONLY', 50, 780, { align: 'center', width: 495 });
    doc.end();
};
