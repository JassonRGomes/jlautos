...
import { generateGoogleCalendarLink, generateICSContent } from '../services/calendarService';
import {
  sendTestDriveCreatedEmail,
  sendTestDriveApprovedEmail,
  sendTestDriveModifiedEmail,
  sendTestDriveCancelledEmail,
  sendTestDriveRejectedEmail,
} from '../services/emailService';
import {
  sendTestDriveCreatedSMS,
  sendTestDriveApprovedSMS,
  sendTestDriveModifiedSMS,
  sendTestDriveCancelledSMS,
  sendTestDriveRejectedSMS,
 ...
 */
const isWithinBusinessHours = (dateInput: Date | string, timeStr: string): boolean => {
...
const generateBookingReference = async (): Promise<string> => {
  ...
  while (!isUnique) {
    ...
    const existing = await prisma.testDriveBooking.findUnique({
...
export const createBooking = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    const duplicate = await prisma.testDriveBooking.findFirst({
      where: {
        customerId: user.id,
        vehicleId,
    ...
    await sendTestDriveCreatedEmail(mailData);
    if (booking.customer.phone) {
      await sendTestDriveCreatedSMS(bookingReference, booking.customer.phone);
    }
...
export const deleteBooking = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    await sendTestDriveCancelledEmail(mailData);
    if (booking.customer.phone) {
      await sendTestDriveCancelledSMS(booking.bookingReference, booking.customer.phone);
    }
 ...
 */
export const getDealerBookings = async (req: AuthenticatedRequest, res: Response) => {
...
export const rejectBooking = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    await sendTestDriveRejectedEmail(mailData);
    if (booking.customer.phone) {
      await sendTestDriveRejectedSMS(booking.bookingReference, booking.customer.phone);
    }
 ...
 */
export const cancelBookingDealer = async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
 ...
 */
export const modifyBooking = async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
  ...
  try {
    ...
    await sendTestDriveModifiedEmail(mailData);
    if (updated.customer.phone) {
      await sendTestDriveModifiedSMS(updated.bookingReference, updated.customer.phone);
    }
 ...
 */
export const completeBooking = async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
  const user = req.user!;
 ...
 */
export const deleteBookingDealer = async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
  const user = req.user!;
